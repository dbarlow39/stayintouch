...
interface LoanApplicationLetterViewProps {
...
const LoanApplicationLetterView = ({ propertyData, propertyId, onBack, onEdit, onNavigate }: LoanApplicationLetterViewProps) => {
  ...
  // Lender first name
  const lenderFirstName = propertyData.lendingOfficer
  ...
  const handleCopyLenderLetter = () => {
    const subject = `Loan Application Verification for ${propertyData.streetAddress}`;
    const recipients = propertyData.lendingOfficerEmail || "";
    copyAndEmail('loan-app-lender-letter-content', subject, recipients);
  };

  const handleCopySellerLetter = () => {
    const subject = `Loan Application Received`;
    const recipients = propertyData.sellerEmail || "";
    copyAndEmail('loan-app-seller-letter-content', subject, recipients);
  };
  ...
  const navigationItems = [
    ...
    { label: "Loan Application", icon: FileText, onClick: () => {}, active: true },
  ];
  ...
  return (
    <div className="flex w-full min-h-[600px]">
      ...
      <div className="flex-1 py-8 px-4">
        {/* Letter 1: To Lender */}
        <div className="max-w-4xl mx-auto" id="loan-app-lender-letter-content">
          <div className="flex items-center justify-between mb-8 print:mb-4">
            <div className="flex items-center gap-3">
              <img src={logo} alt="Sell for 1 Percent" className="h-16 w-auto print:h-12" />
              <div>
                <h1 className="text-3xl font-bold text-foreground">Loan Application Verification</h1>
                <p className="text-muted-foreground">Notice to {propertyData.lendingOfficer || "Lender"}</p>
              </div>
            </div>
            <div className="flex gap-2 print:hidden no-pdf">
              <Button onClick={handleCopyLenderLetter} size="lg" className="bg-emerald-600 hover:bg-emerald-700 text-white">
          ...
          <Card className="p-8 mb-6 print:shadow-none">
            <div className="prose prose-lg max-w-none text-foreground">
              <p className="mb-4">Hi {lenderFirstName},</p>
              <p className="mb-4">
                We are coming up on the Loan Application deadline for your client {buyerFullName} purchasing the property I represent at {fullAddress}. If you would confirm the buyer has made their official application with you I would appreciate it.
              </p>
        ...
        {/* Letter 2: To Seller/Homeowner */}
        <div className="max-w-4xl mx-auto mt-10" id="loan-app-seller-letter-content">
          <div className="flex items-center justify-between mb-8 print:mb-4">
            <div className="flex items-center gap-3">
              <img src={logo} alt="Sell for 1 Percent" className="h-16 w-auto print:h-12" />
              <div>
                <h1 className="text-3xl font-bold text-foreground">Loan Application Received</h1>
                <p className="text-muted-foreground">Notice to {propertyData.name || "Homeowner"}</p>
              </div>
            </div>
            <div className="flex gap-2 print:hidden no-pdf">
              <Button onClick={handleCopySellerLetter} size="lg" className="bg-emerald-600 hover:bg-emerald-700 text-white">
                <Copy className="mr-2 h-4 w-4" />
                Copy & Email
              </Button>
            </div>
          ...
          <Card className="p-8 mb-6 print:shadow-none">
            <div className="prose prose-lg max-w-none text-foreground">
              ...
              <p className="mb-4">
                Just a quick note to confirm the buyer has made their formal application with {propertyData.lenderName || "the lender"} to purchase your home.
              </p>
              <p className="mb-4">Everything is progressing just the way it should. Let me know if you have any questions.</p>
              <p className="mb-4">Thanks</p>
              <p className="mb-4">{agentFirstName}</p>
              {agentBio ? (
                /<[a-z][\s\S]*>/i.test(agentBio) ? (
...
export default LoanApplicationLetterView;
