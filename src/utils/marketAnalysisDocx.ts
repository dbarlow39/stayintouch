...
function sectionHeading(text: string): Paragraph {
  ...
  return new Paragraph({
    ...
    children: [
      new TextRun({ text: titleCase, bold: true, color: DARK_SCARLET, font: "Arial", size: 26 }),
    ],
  });
}
...
export async function generateMarketAnalysisDocx(
  ...
  // ── 8. ZILLOW ZESTIMATE (only if data exists) ──
  const hasZillowContent = zillowImage || narrative.zillowWordOn || narrative.zillowNoteOn;
  if (hasZillowContent) {
