...
interface ImportantDatesEmailPayload {
  to_email: string;
  from_name: string;
  reply_to: string;
  client_name: string;
...
serve(async (req) => {
  ...
  try {
    const payload: ImportantDatesEmailPayload = await req.json();
    const {
      to_email, from_name, reply_to,
      client_name, street_address, dates_html, sections_html,
...
<body style="margin: 0; padding: 0; background-color: #f9fafb; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f9fafb; padding: 24px 0;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td style="text-align: right; vertical-align: middle;">
                    <span style="font-size: 22px; font-weight: 700; color: #1f2937;">Important Dates</span><br/>
                    <span style="font-size: 13px; color: #6b7280;">Keep Track of Your Closing Timeline</span>
                  </td>
                </tr>
              </table>
            </td>
    ...
    const emailBody: Record<string, unknown> = {
      ...
      reply_to: reply_to,
      to: [to_email],
      subject: `Important Dates for Your Property Sale - ${street_address}`,
      html: html,
    };
    ...
    if (!res.ok) {
      console.error('[send-important-dates-email] Resend error:', result);
    ...
    console.error('[send-important-dates-email] Error:', err);
